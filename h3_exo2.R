...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      # Input: Slider for step
      sliderInput(inputId = "sequence",
                  label = "Step:",
                  min = 1,
                  max = 100,
                  value = 1,
    ...
    mainPanel(
      ...
      # Output: Histogram ----
      plotOutput(outputId = "matteoPlot")
...
server <- function(input, output) {
  ...
  output$matteoPlot <- renderPlot({
    ...
    ggplot(data = data_positions, aes(x = x, y = y, color = name)) +
